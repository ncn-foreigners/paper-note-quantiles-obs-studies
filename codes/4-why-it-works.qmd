...
result_sim <- replicate(n = 500, simplify = FALSE, expr = {
...
```{r}
saveRDS(result_sim_df, file = "../results/sim-3-why-it-works-eb.R")
```
...
result_sim_df |> 
  transform(variable = factor(variable, paste0("m",1:5),
            ...
            group = factor(group, c("y1", "y2", "y3"),
                           c("Y1 (lin)", "Y2 (med nonlin)", "Y3 (str nonlin)"))) |> 
...
ggplot(data = models1_df[group == "y3"], aes(x = y, y = value)) +
  ...
  labs(x = "Y observed", y = "Y predicted", 
       title = "Scatter plots for Y3 (strong nonlinearity)") +
...
Add plots from VGAM to show linear approximation in deciles

```
m3 <- vgam(formula = y3 ~ s(x1) + s(x2)  + s(x3) + s(x4) + s(x5), data= df2, family = uninormal())
m3 <- vgam(formula = y3 ~ s(x1) + s(x2)  + s(x3) + s(x4) + s(x5) + 
                          s(x1*x2)+ s(x1*x5) + s(x2*x5) + s(x1*x2*x5), 
data= df2, family = uninormal())
```
