...
for (b in 1:B) {
  ...
  res_ra <- tryCatch(joint_calib_att(formula_means = ~ X5 + X6, #X1 + X2 + X3 + X4 + X5 + X6,
  ...
  if (class(res_ra) != "jointCalib") next
  
  res_eb <- joint_calib_att(formula_means = ~ X5 + X6, #X1 + X2 + X3 + X4 + X5 + X6,
                            formula_quantiles = ~ X1 + X2 + X3 + X4,
                            treatment = ~ W,
                            data = sim_df,
                            probs = seq(0.1, 0.9, 0.1),
  ...
  res_el <- joint_calib_att(formula_means = ~ X5 + X6, #X1 + X2 + X3 + X4 + X5 + X6,
                            formula_quantiles = ~ X1 + X2 + X3 + X4,
                            treatment = ~ W,
                            data = sim_df,
                            probs = seq(0.1, 0.9, 0.1),
  ...
  result <- rbind(y_treat - y_cont_naive, y_treat - y_cont_jra, y_treat - y_cont_jeb, y_treat - y_cont_jel, y_treat - y_cont_eba)
