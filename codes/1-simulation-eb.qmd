...
for (b in 1:B) {
  ...
  for (des in c("d1", "d2", "d3")) {
    ...
    y1_res <- df_long_des[, lapply(.SD, 
                         ...
                         .SDcols = patterns("w_")][, ":="(var="y1", est="ate", tau=NA)]
    y2_res <- df_long_des[, lapply(.SD, 
                         ...
                         .SDcols = patterns("w_")][, ":="(var="y2", est="ate", tau=NA)]
    ...
    y3_res <- df_long_des[, lapply(.SD, 
                         ...
                         .SDcols = patterns("w_")][, ":="(var="y3", est="ate", tau=NA)]
    ...
    y1_res_q <- df_long_des[, lapply(.SD, 
                         ...
                         .SDcols = patterns("w_")][, ":="(var="y1", est="qte", tau=tau)]
    ...
    y2_res_q <- df_long_des[, lapply(.SD, 
                         ...
                         .SDcols = patterns("w_")][, ":="(var="y2", est="qte", tau=tau)]
    ...
    y3_res_q <- df_long_des[, lapply(.SD, 
                         ...
                         .SDcols = patterns("w_")][, ":="(var="y3", est="qte", tau=tau)]
...
result_total_df <- rbindlist(result_total, idcol = "b") |> 
  melt(id.vars = c("b", "design", "var", "est", "tau"), variable.name = "weight", value.name = "value")
```
...
```{r}
result_total_df[est == "qte", 
                .(rmse = mean(value)), #sqrt(mean(value)^2 + var(value))), 
                .(design, var, weight, tau)] |> 
  ggplot(data = _, aes(x = tau, y = rmse, linetype = weight)) +
  geom_line() + 
  facet_grid(var~design, scales = "free_y")
```
