...
for (i in 1:reps) {
  set.seed(2023 + i)
  if (i %% 10 == 0) print(i)
  data_sim1 <- cbps_sim_data(n = n_sample, dgp=2, y_nonlin = TRUE)
  xcov <- cbind(1, data_sim1[, c("X1", "X2", "X3", "X4")])
  ...
  ## proj ### THIS SHOULD BE CHANGED TO IPS_PROJ!!!!!! this is only for experimental result
  fit.proj <- IPS::IPS_ind(d = data_sim1$Treat,
                          x = xcov,
                          beta.initial = fit.cbps$coefficients, maxit = 25000)
