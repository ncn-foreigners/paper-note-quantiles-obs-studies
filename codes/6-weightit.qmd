...
fit1 <- lm_weightit(re78 ~ treat, data = LaLonde,
                    weightit = ebal_result1)
summary(fit1)
```

```{r}
fit2 <- lm_weightit(re78 ~ treat, data = LaLonde,
                    weightit = ebal_result2)
summary(fit2)
```
...
Estimate standard errors for both approaches

```{r}
fit_cbps1 <- lm_weightit(re78 ~ treat, data = LaLonde,
                         weightit = cbps_result1)
summary(fit_cbps1)
```

```{r}
fit_cbps2 <- lm_weightit(re78 ~ treat, data = LaLonde,
                         weightit = cbps_result2)
summary(fit_cbps2)
```
